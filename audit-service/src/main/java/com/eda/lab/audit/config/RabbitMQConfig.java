package com.eda.lab.audit.config;
 ...
 * - Main Queue: document.audit.q (consumes ALL document events)
 * - Routing Key Pattern: "document.*" (wildcard to match all document events)
 * - Dead Letter Exchange: doc.dlx
 * - Dead Letter Queue: document.audit.dlq (for failed messages after retries)
 * 
 * Events Consumed:
 * - DocumentUploaded (document.uploaded)
 * - DocumentValidated (document.validated)
 * - DocumentRejected (document.rejected)
 * - DocumentEnriched (document.enriched)
 * 
 * Retry Strategy:
 * - maxAttempts: 5
...
public class RabbitMQConfig {
    ...
    public static final String AUDIT_QUEUE = "document.audit.q";
    public static final String ROUTING_KEY_PATTERN = "document.*";  // Wildcard for all document events
    
    // Dead Letter Configuration
    public static final String DLX_EXCHANGE = "doc.dlx";
    public static final String DLQ_QUEUE = "document.audit.dlq";
    public static final String DLQ_ROUTING_KEY = "document.audit.dlq";
    ...
    /**
     * Main queue for consuming ALL document events.
    ...
    /**
     * Bind audit queue to topic exchange with wildcard pattern "document.*".
     * This ensures audit-service receives ALL document events:
     * - document.uploaded
     * - document.validated
     * - document.rejected
     * - document.enriched
     */
    @Bean
    public Binding auditQueueBinding(Queue auditQueue, TopicExchange topicExchange) {
     ...
     * Duplicate events (idempotency) should NOT throw exceptions.
     */
    @Bean
