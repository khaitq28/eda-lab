 ...
 * 
 * Key Method:
 * - findPendingEvents(): Get events ready to publish (with row-level locking for multi-instance safety)
 * 
 ...
 *     List<OutboxEvent> pending = outboxEventRepository.findPendingEvents(Instant.now(), 50);
 *     
 ...
 *         } catch (Exception e) {
 *             event.markAsFailed(e.getMessage(), 10);
 *             outboxEventRepository.save(event);
 *         }
 *     }
 * }
